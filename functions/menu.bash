...
show_main_menu() {
  ...
  if [[ "$choice" == "" ]]; then
    ...
    ver="${evccForcePkg:-${evccpkgversion}}"
    if ! cond_redirect echo -n n | apt install -y --allow-downgrades "evcc${ver+=$ver}"; then echo "FAILED (EVCC package installation)"; fi
    openhab_setup "openHAB" "${repo:-release}" "${openhabForcePkg:-${openhabpkgversion:-4.2.2-1}}"
    upgrade_ems
