...
upgrade_ems() {
  ...
  mode=${1}
  if [[ -n "$INTERACTIVE" ]]; then
    if ! whiptail --title "EMS komplettes Update" --yes-button "EMS-Config überschreiben" --cancel-button "Abbrechen" --defaultno --yesno "$introText" 19 80; then echo "CANCELED"; return 1; fi
    mode=full
  fi
  ...
  if [[ "$mode" == "full" ]]; then
    ...
    ln -sf . conf
    unzip -o "$temp" conf/things\* conf/items\* conf/rules\* conf/transform\* conf/yaml\* conf/html\* conf/scripts\*
    rm -f conf )
  fi
